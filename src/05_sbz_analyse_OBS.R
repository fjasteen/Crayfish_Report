...
## --- Ruimtelijke lagen opladen ----
# Vlaanderen is referentie-CRS
...
gemeenten     <- st_read(file_gemeenten, quiet = TRUE) %>% clip_to_vlaanderen()

##  Natura 2000 aquatische habitats
...
##  Habitatrichtlijngebieden met aquatische habitats 
...
## --- analysedataset opladen ----

# Laad de analyse dataset
CF_data <- readr::read_csv(file_analyse_dataset_rapport, show_col_types = FALSE)
CF_data_sf <- CF_data %>%
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326) %>%
  st_transform(target_crs)


## --- baseplot construeren ----
...
# Helper functie om kaarten te plotten
plot_protected_map <- function(base_layer, points_layer, outfile, 
                               fill_col = "darkgreen", alpha = 1) {
  ...
  ggsave(p, file = outfile, width = 15, height = 8, units = "cm", dpi = 400)
}
...
CF_long <- CF_data %>%
  select(
    dat.source,
    sample_id,          # of locID / Code.sample.location
    date,
    Longitude, Latitude,
  ...
  mutate(
    presence = case_when(
      presence_raw == 1 ~ 1,      # aanwezigheid  
      presence_raw == 0 ~ 0,      # echte afwezigheid
      TRUE ~ NA_real_             # onbekend / geen data
    )
  ) %>%
  select(-presence_raw)
...
plot_protected_map(
  ...
  points_layer = sf_presence,
  outfile = "~/SWO craywatch/R/output/maps/map_natura_2000.png"
)

# Habitatrichtlijngebieden (HBTRL)
plot_protected_map(
  base_layer = hbtrl,
  points_layer = sf_presence,
  outfile = "~/SWO craywatch/R/output/maps/map_hbtrl.png"
)

# Soortenbeschermingsprogramma's
plot_protected_map(
  base_layer = bind_rows(
    sbp_pgs_aq %>% mutate(type = "PGS"),
    sbp_vissen %>% mutate(type = "Vissen")
  ),
  points_layer = sf_aanwezigheid,
  outfile = "~/SWO craywatch/R/output/maps/map_sbp.png",
  fill_col = "darkgreen",
  alpha = 0.8
)
...
for (soort in unieke_soorten) {
  species_data <- CF_presence %>%
    filter(species == soort) #filter de presence data voor soort
  
  # Buffer van 100 m rond de occ
  ...
  if (nrow(occ_buf_100) > 0) {
    ...
    # Maakt het intersect om te weten welke punten in de wolken zitten (TRUE/FALSE)
    # Krijgt matrix van #obs x #wolken
    species_points_in_union <- species_data %>% st_transform(CRS_PROJECTED) %>% st_intersects(occ_data_union, sparse = FALSE)
    
    #  Haalt minimum datum uit species_points in union (dus eerst obs voor eerste waarneming)
