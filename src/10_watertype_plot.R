...
# Data = Craywatch (deler is totaal aantal bemonsteringen, niet beschikbaar voor
# GBIF data)
...
# 4. GLMM
# ==============================================================================
# We gebruiken alleen Craywatch data (vanwege de 'nullen')

model_data <- cray_long %>%
  filter(dat.source == "craywatch_data") %>% 
  filter(!is.na(presence)) %>%
  mutate(
    # Uniek ID voor waterlichaam (Random Effect)
    Waterbody_ID = coalesce(as.character(VHAG), as.character(WVLC), as.character(locID))
  )

if(nrow(model_data) > 0) {
  message("Start GLMM analyse (Stilstaand vs Stromend)...")
  
  # Model: Aanwezigheid ~ Watertype * Soort + (1|Waterlichaam)
  glmm_water <- glmer(
  ...
  # Optioneel: Type III Anova
  # print(car::Anova(glmm_water, type = "III"))
  
} else {
  warning("Te weinig Craywatch data voor GLMM analyse.")
}